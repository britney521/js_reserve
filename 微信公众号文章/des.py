...
headers = {
    ...
    # 'cookie': 'ua_id=OScCF1OgfY29aGgAAAAAAApcQYkiUSA1KG4_25pJj6U=; wxuin=53798001561140; mm_lang=zh_CN; RK=hQetsOsfZb; ptcz=54ba76151d84eb91a774d0772c386b65ccd50986f06bd442bd2112ce462d9359; rewardsn=; wxtokenkey=777; uin=o786120564; _ga=GA1.1.1007850669.1759283690; _ga_PF3XX5J8LE=GS2.1.s1759283690$o1$g0$t1759283692$j58$l0$h0; _clck=xsf8ts|1|g04|0; uuid=b81a5768034db956f683af834449de96; rand_info=CAESINFKGFdT2z/2d8F512yDcYSo5P3xmMcQg+hlsTTWlp7O; slave_bizuin=3881830519; data_bizuin=3881830519; bizuin=3881830519; data_ticket=gV630cMMx90KueXNzD3fNi7lPWBncZTLApZyIXw8UBH+fW+q6GZpzE2SmVurT50i; slave_sid=WFcxOGpoT3NsZmJXZTRIZnNGMVdmSWVqMlZaMHFVdHRZZmYyQmpvSjBiSXVPNUgzMVpjMTlpVnB0ZlN4bWJqZDduemdZYkVIXzBOXzgwbXRNQkxmeUM5aHhxdGd6Rl9xOWxEam5VNFdWUDdZTFRwMXdpdFBRWUlwMmZUekRKZ0t0NmFuMEYxM2EyM2w0OEFK; slave_user=gh_67766ea540c0; xid=110cfd25789d0c982541b20f5cecfe17; mmad_session=6b1c5ea73566c93ec326cc7746aac97259d28200e3b651df5b9da91a7a044069c5560df512c4e7cf3dfbaec9b6f5fba6d559640665e98cb53ae1726ecd179211de6a01cbad7e1e258074170e7e08681d8a0874a622195baf0f80717cb0690c6711de1c56c245721266e7088080fefde3; pgv_info=ssid=s6817715176; ts_last=mp.weixin.qq.com/cgi-bin/frame; pgv_pvid=590291630; ts_uid=3670827444; _clsk=13gqres|1760344684167|6|1|mp.weixin.qq.com/weheat-agent/payload/record',
    'pragma': 'no-cache',
    'priority': 'u=1, i',
    'referer': 'https://mp.weixin.qq.com/cgi-bin/appmsg?t=media/appmsg_edit_v2&action=edit&isNew=1&type=77&createType=8&token=683909134&lang=zh_CN&timestamp=1760343818162',
...
f = open('{}.csv'.format(query), 'w',encoding='utf-8',newline='')
csv_w = csv.writer(f)
csv_w.writerow(['标题','链接','封面','内容','推出日期'])
...
for i in range(1,101):
    ...
    publish_page = json.loads(response.json()['publish_page'])
    publish_list = publish_page['publish_list']
    for publish in publish_list:
        publish_info = publish['publish_info']
        # pprint.pprint(json.loads(publish_info), sort_dicts=False)
        appmsgex = json.loads(publish_info)['appmsgex'][0]
        ...
        content = get_detail(link)
        print(appmsgid,title,cover,update_time)
        csv_w.writerow([title,link,cover,content,update_time])
